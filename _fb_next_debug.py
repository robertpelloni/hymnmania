...
def main():
    ...
    with sync_playwright() as pw:
        ...
        r0 = fb.evaluate("""() => {
            ...
            for (var i = 0; i < els.length; i++) {
                ...
                if (t === 'Create reel' && e.offsetParent) {
                    ...
                    if (rr.y > 40 && rr.y < 150) return JSON.stringify({x: Math.round(rr.x+rr.width/2), y: Math.round(rr.y+rr.height/2)});
                }
            }
            return 'none';
        }""")
        ...
        print("file set")
        # wait copyright clear
        for i in range(20):
            fb.wait_for_timeout(5000)
            body = fb.evaluate("document.body.innerText")
            if "checking for copyrighted" not in body.lower():
                print(f"ready at {i*5}s")
                break
        # Now find the composer's Next button — look within the 'Create reel' container
        info = fb.evaluate("""() => {
            // find the element whose text starts with 'Create reel' or contains 'Replace video'
            var container = null;
            var els = Array.from(document.querySelectorAll('div'));
            for (var i = 0; i < els.length; i++) {
                var e = els[i];
                var t = (e.innerText||'').trim();
                if (t.includes('Replace video') || t.includes('or drag and drop')) {
                    container = e; break;
                }
            }
            if (!container) return JSON.stringify({found: false});
            // find buttons inside this container
            var btns = Array.from(container.querySelectorAll('div[role=button],button')).filter(x=>x.offsetParent).map(x=>(x.innerText||'').trim());
            var r = container.getBoundingClientRect();
            return JSON.stringify({found: true, x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), btns: [...new Set(btns)].slice(0,10)});
        }""")
        print("composer:", info)
        b.close()
