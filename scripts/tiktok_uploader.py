...
def upload_via_playwright(
    video_path, caption, is_aigc=True, privacy="PUBLIC_TO_EVERYONE"
):
    ...
    with sync_playwright() as p:
        ...
        caption_input = page.locator(
            "[data-text='true'], public-DraftEditor-content, [role='textbox']"
        ).first
        if caption_input:
            caption_input.click()
            time.sleep(1)
        ...
        try:
            ...
            if post_btn.is_visible():
                ...
                time.sleep(10)
                page.screenshot(
                    path=os.path.join(ROOT, "..", "tiktok_upload_result.png")
                )
...
def main():
    ...
    parser.add_argument(
        "--aigc", action="store_true", default=True, help="Mark as AI-generated"
    )
    parser.add_argument("--privacy", default="PUBLIC_TO_EVERYONE")
    args = parser.parse_args()

    title = args.title or os.path.basename(args.video).replace(".mp4", "").replace(
        "_", " "
    )
