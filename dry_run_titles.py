...
def detect_speed(t):
    if "05x" in t or "0.5x" in t or "half" in t: return "Half-Speed (0.5x)"
    if "15x" in t or "1.5x" in t: return "1.5x Speed"
    if "20x" in t or "2.0x" in t: return "Double Speed (2.0x)"
    if "30x" in t or "3.0x" in t or "300bpm" in t: return "Triple Speed (3.0x)"
    return "1.0x Speed"

def detect_variant(t):
    if t.endswith(" b") or " b_" in t or t.rstrip().endswith("b"): return " [B]"
    if t.endswith(" a") or " a_" in t or t.rstrip().endswith("a"): return " [A]"
    return ""

def build_correct_title(title):
    orig = title.lower()
    if "hymn 2026 remix:" in orig and " | " in orig: return None
    if "classical remix" in orig and " | " in orig: return None
    genre = detect_genre(orig)
    speed = detect_speed(orig)
    variant = detect_variant(orig)
    for key, (name, author, year) in CLASSICAL.items():
        if key in orig:
            if not genre: genre = "Electronic"
...
def list_all():
    ...
    page_token = None
    while True:
        req = yt.search().list(part="snippet", forMine=True, maxResults=50, type="video", order="date", pageToken=page_token)
        resp = req.execute()
