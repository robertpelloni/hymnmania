...
GENERATE_ENDPOINT = "/api/songs/generate"
...
class UdioAPIClient:
    ...
    def is_available(self):
        ...
        try:
            ...
            # Try to fetch user songs as a lightweight connectivity test
            resp = requests.get(
                f"{self.base_url}/api/songs/me?pageSize=1",
                headers=headers,
                timeout=10,
            )
    ...
    def generate(self, prompt, style=None, title=None, custom_mode=True):
        ...
        # Combining style and prompt for the Udio prompt
        full_prompt = f"{style}, {prompt}" if style else prompt
        
        payload = {
            "prompt": full_prompt,
            "lyrics_type": "user" if custom_mode else "generate",
            "custom_lyrics": prompt if custom_mode else None,
            ...
            "samplerOptions": {
                "seed": -1,
                "bypass_prompt_optimization": True  # Manual Mode
            },
            "is_instrumental": True if not custom_mode else False
        }
    ...
    def get_song_status(self, song_id):
        """Get the status of a specific song.
        ...
        """
        headers = self._get_headers()
        url = f"{self.base_url}{SONGS_ENDPOINT}/{song_id}"
    ...
    def poll_until_ready(self, song_id, interval=None, timeout=None):
        """Poll the Udio API until the song is ready.

        Args:
            song_id (str): The ID of the song to poll.
            interval (int): Seconds between checks.
            timeout (int): Max seconds to wait.

        Returns:
            str: URL of the ready audio file.
        """
        ...
        while time.time() - start_time < timeout:
            status_data = self.get_song_status(song_id)
            if status_data:
                state = status_data.get("state")
                if state == "finished":
                    return status_data.get("audio_url")
                elif state in ("failed", "error"):
                    raise RuntimeError(f"Udio generation failed for song {song_id}")
            
            time.sleep(interval)
        
        raise TimeoutError(f"Udio generation timed out for song {song_id}")
