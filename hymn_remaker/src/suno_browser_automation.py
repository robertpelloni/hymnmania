...
class SunoBrowserAutomation:
    ...
    def _get_active_tab(self, require_suno=False):
        ...
        if suno_targets:
            ...
            logger.info(f"Selected Suno tab: {res_tab.get('url')} (ID: {res_tab.get('id')})")
            return res_tab
    ...
    def trigger_generation(self, prompt, audio_path=None, make_instrumental=True):
        ...
        if audio_path and os.path.exists(audio_path):
            ...
            try:
                ...
                if nodes['result']['nodeIds']:
                    ...
                    logger.info("Suno: Audio file injected. Waiting for upload to complete (25s)...")
        ...
        (function() {{
            ...
            // Try to find the style textarea specifically
            const ta = textareas.find(el => (el.placeholder || '').toLowerCase().includes('style') && el.offsetParent !== null) || textareas[0];
        ...
        """
        logger.info(f"Suno: Prompt injection result: {self.execute_js(ws_url, prompt_js)}")
        ...
        if make_instrumental:
            ...
            """
            logger.info(f"Suno: Instrumental check: {self.execute_js(ws_url, instr_js)}")
