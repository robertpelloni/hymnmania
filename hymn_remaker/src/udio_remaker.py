...
class UdioRemaker:
    ...
    def remake(self, wav_path, prompt, variance=0.35, mode="auto", prompt_strength=0.65, manual_mode=True, extension_hack=False):
        ...
        try:
            if self.client: return self.remake_api(wav_path, prompt, variance=variance)
            else: return self.remake_edge(wav_path, prompt, variance=variance, prompt_strength=prompt_strength, manual_mode=manual_mode, extension_hack=extension_hack)
        except Exception as e:
            if mode == "auto":
