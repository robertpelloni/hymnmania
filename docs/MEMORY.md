...
- **Real-time Playback & Mixing:** The native C++ engine (`hymn_player_ext`) has been enhanced with a real-time `fluid_audio_driver` and supports real-time multi-channel mixing (`set_channel_volume`) for the Studio interfaces.
...
- **FFmpeg for Audio-Visuals:** FFmpeg remains the backbone for final assembly, vocal processing, and creating audio-reactive visuals.
